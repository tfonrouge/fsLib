...
— their memory + SQL conformance tripwires are now live green; the Mongo runtime pins are wired via
the Testcontainers suite (D11) and turn live with the first CI run.
...
P1.1–P1.7 and P1.9 are **done and verified**. The P1.8 conformance suite is **complete** across all
three engines: memory + SQL run live (gate I5, permission parity I6, validation side-effect I2, delete
safety I3, hook order I1, init lifecycle I4), and **Mongo participation (C) is now added** via a
Testcontainers `MongoConformanceTest` (Docker-assume-skip — green-or-skip locally, runs the Mongo
engine for real in CI; D11), pinning the P2.1/P2.2/P2.3 Mongo convergence (I1/I3/I4) **plus the
D6 real-mongod write-failure path** (F1 duplicate-key, `MongoWriteFailureTest`). It is
compile-verified + skip-clean locally; **the first real Mongo green runs in CI** (no local Docker).
Phase 2 is complete in code (P2.1/P2.2/P2.3). The remaining work is the **`4.0.0` version
bump/release packaging**, applied as a separate commit when this batch is released.
