...
**P2.1 (delete de-dup) is done** — its SQL exactly-once conformance tripwire is now live green;
**P2.2 (hook-order convergence) and P2.3 (`onAfterOpen` lifecycle) remain**, each needing its own
deliberate approval.
...
asserted on both engines, with the target hook-order invariant (I1) assume-skipped on SQL until
P2.2. The only P1.8 remainder is **Mongo participation (C)** — the
real-mongod write-failure/cross-engine coverage — pending the Docker (Testcontainers) vs flapdoodle
decision. Phase 2 (BREAKING — P2.2 hook-order convergence, P2.3 `onAfterOpen`) is
