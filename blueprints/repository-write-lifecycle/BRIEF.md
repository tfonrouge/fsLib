...
|----|----------|--------|---------|
| F1 | High | Fixed (P1.1, P1.2) | `Coll.insertOne` wrote a phantom changelog on failed/validation-rejected insert (fixed P1.1). `updateOne`/`updateFieldsById` now hoist `onValidate`+strip above the write `try` (P1.2) — safe by structure, not coincidence. The D6 real-mongod duplicate-key regression test landed: `MongoWriteFailureTest` in `:conformance` (Testcontainers, CI; D11). |
...
| F3 | High | Fixed (P2.2) | Create/update before-hook + query-gate order converged to **upsert-outermost** across engines (Mongo `updateOne`/`updateFieldsById`, SQL `updateOne`). Memory + SQL pinned by conformance (`canonicalHookOrderOn{Create,Update}`); Mongo now exercised by the suite **in CI** (Testcontainers, D11 — skips locally). Known upsert-insert I1 gap logged in CONTRACT. |
...
| F5 | High | Fixed (P2.3) | Generic item/list entry points now call retryable `ensureOpen()` in all engines. Mongo no longer launches init from the constructor; `open()` awaits `with(coroutine) { onAfterOpen(); indexes() }` and surfaces failures. Memory + SQL init lifecycle is pinned by conformance; Mongo now exercised by the suite **in CI** (Testcontainers, D11 — skips locally). |
...
| N3 | Medium | Addressed (P1.9) | The low-level/service path is implicitly **permission-trusted** when `call == null` (`Coll.getCrudPermission(apiItem)`'s `call == null` short-circuit). SQL now enforces the per-action CRUD permission on the remote path (P1.9); memory is intentionally permission-free (samples/tests). Documented in CONTRACT I6. |
...
| N5 | Medium (cross-link) | Addressed | The blessed service tier writes exclusively through `updateFieldsById`/`updateOne`. F1's residue on those methods is fixed (P1.2) and F3's hook-order divergence is fixed (P2.2) — the supported hot path now follows the canonical order. |
