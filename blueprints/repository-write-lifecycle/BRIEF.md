...
| F3 | High | Fixed (P2.2) | Create/update before-hook + query-gate order converged to **upsert-outermost** across engines (Mongo `updateOne`/`updateFieldsById`, SQL `updateOne`). Memory + SQL pinned by conformance (`canonicalHookOrderOn{Create,Update}`); Mongo by code + cross-engine symmetry (runtime pin pending C). Known upsert-insert I1 gap logged in CONTRACT. |
...
| F5 | High | Fixed (P2.3) | Generic item/list entry points now call retryable `ensureOpen()` in all engines. Mongo no longer launches init from the constructor; `open()` awaits `with(coroutine) { onAfterOpen(); indexes() }` and surfaces failures. Memory + SQL init lifecycle is pinned by conformance; Mongo runtime pin pending C. |
