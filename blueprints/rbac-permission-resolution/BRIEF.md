...
| R8 | High (unverified security code) | Open | The entire resolution engine is untested: `getGroupPermission` has **0 callers** in any test; `permissionState` is **never invoked** by a test. The lone permission test (`perActionPermissionParity`) installs a deny-provider but only actually asserts for **SQL** — the Mongo (`enforcesPermissions=false`) and InMemory (`false`) conformance profiles structurally can't reach the real engine. No sample app wires RBAC at all. → P1 characterization. |
