...
|----|----------|--------|---------|
| R1 | High (abstraction leak) | **Largely addressed (P3.1a/b, P3.2a/b)** | Was: group + single-action resolution lived only in `mongodb/IRoleInUserColl`. **P3.1a** extracted the resolution **algebra** into the backend-agnostic `RbacResolver` over `IRbacGrantPort` (Mongo is one port impl); the policy is no longer Mongo-private. **P3.1b** added the **InMemory** native port (`InMemoryRbacGrantPort`) — the resolver runs over two real ports. **Registration fixed:** the construction side-effect is replaced by the explicit `MongoRbac.register` boot registrar (D10/P3.2a), and **P3.2b** unified Mongo's dispatch onto the registered `PermissionRegistry` provider (deleting the `Coll.roleInUserColl` companion + the duplicate `CollPermission`) — the global split-brain is gone and the harness drives the live Mongo path. **Remaining R1 gap:** native **SQL** RBAC port (needs RBAC tables; a separate sub-blueprint) and the agnostic surface still carrying only `getCrudPermission` (single-action/group join it in **P3.2c**). → D5/D10. |
...
| R8 | High (unverified security code) | **Largely addressed (P1.1, P3.2b, P3.3)** | Was: the entire resolution engine was untested (`getGroupPermission` had **0** test callers; `permissionState` was never invoked). P1.1's `RbacPermissionResolutionCharacterizationTest` invokes `permissionState` and reaches `getGroupPermission` (C1–C4 frozen); the no-DB `RbacResolverTest` (13) + `RbacMembershipTest` (6) pin the resolver/membership algebra; **P3.2b** flipped Mongo `enforcesPermissions=true` so `perActionPermissionParity` + the fail-closed test now assert for **SQL and Mongo**; and **P3.3** added the `samples/rbac` walkthrough that wires user + group grants end-to-end (closing the "no sample wires RBAC" gap). **Remaining gap:** only the optional full `upVote`/`Allow`-side + SingleAction characterization matrix. (C5 side-effect pinned — P1.2 `c5a09a6c` + the P2.3 flip.) → P1.1/P3.2b/P3.3 done; the optional matrix gap remains. |
...
| R10 | Low (convention/robustness) | **Registration half: D10 locked, P3.2a shipped (`MongoRbac.register`); naming half: deferred** | Two parts. **(1) Registration:** provider registration is a **construction side effect** on a global mutable (`Coll.kt:1740-1744`, sets **both** `Coll.roleInUserColl` and `PermissionRegistry.rolePermissionProvider`). **D10 (2026-06-22)** locks the fix = an **explicit boot registrar** (`MongoRbac.register`), shipping in **P3.2a**. **(2) Naming:** the five RBAC colls (`IRoleInUserColl`/`IUserGroupColl`/`IRoleInGroupColl`/`IGroupOfUserColl`/`IAppRoleColl`) are `abstract class`es wearing the `I` interface-prefix — **deferred** to a separate cleanup (out of P3.2a scope; a source-breaking rename). → D5/D10. |
...
`Coll.getCrudPermission`, `IAppRoleColl`), the backend-agnostic seam (`IRolePermissionProvider` /
