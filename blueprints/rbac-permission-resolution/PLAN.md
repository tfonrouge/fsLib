...
| **P3.3** | **Demonstrate RBAC in a sample** (none existed): one sample wiring user + group grants end-to-end, exercising composition + conflict rule. | `samples/rbac/**` | R8 (visibility) | SAFE (additive) | ✅ **done** — new `samples/rbac` console app (`./gradlew :samples:rbac:run`, no DB): seeds `InMemoryRbacGrantPort` and drives the **real** `RbacResolver` + `RbacMembership` to demonstrate the group-only bypass (R12), direct-grant precedence (D1, not a union), existence≠authz (D8), and the D2 tie-break; README shows the production Mongo wiring (`MongoRbac.register` + `ensureRoles`). Runs + verdicts confirmed. |
...
> + the Mongo `groupOnlyMembershipIsSeenByBothOperationsOnMongo` (CI). The P4.5 **sample** is now the
> `samples/rbac` walkthrough (P3.3). **Deferred:** P4.4 (native SQL RBAC — the honest gap).
...
| **P4.5** | **Membership conformance + sample.** Cross-engine membership tests (extend the `enforcesPermissions` seam) exercising **group-assigned, no-direct-row** grants on every claiming engine — the exact bypass scenario; plus the P3.3 sample wires a group-only grant and calls the membership API. | `:conformance`; `samples/**` | R8, R12 | SAFE (additive) | ✅ **done** — conformance: `RbacMembershipTest` (6 InMemory, incl. the group-only bypass) + the Mongo `groupOnlyMembershipIsSeenByBothOperationsOnMongo` (CI). **Sample**: the `samples/rbac` walkthrough (P3.3) seeds a group-only grant and calls the membership API end-to-end. |
...
**Shipped since:** **P3.2a/b** (explicit `MongoRbac.register` registrar + unified Mongo dispatch — the
split-brain is gone, Mongo `enforcesPermissions=true`) and **P3.3** (the `samples/rbac` walkthrough,
closing R8's sample gap and satisfying P4.5's sample).

**Open, in order:** (1) **P3.2c** — bring single-action/group onto the agnostic `IRepository` surface
(`RbacMembership` reachable engine-agnostically); (2) **P4.4** native SQL RBAC port — the honest gap
(separate sub-blueprint; SQL membership stays fail-closed/`Off` until then, D6). The full `upVote`/SingleAction
characterization matrix (R8 tail) remains a smaller optional add.
