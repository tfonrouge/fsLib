...
| F6 | **Launchability is not a view kind — a `ViewItem` is launchable when its URL carries a valid ACTION.** `Read` and `Update` additionally need an id; **`Create` does not** (`urlCreate` = `url + "?action=Create"`; `navigateToQueryCreate(id: ID? = null)`). `ViewHome` has 4 fixed-item destinations (3 `navigateToQueryRead`, 1 `navigateToQueryUpdate`) and no `Create` one — but "create an OT" is exactly what someone types into a palette. *(Corrected 2026-07-16, owner: this first read "a `ViewItem` without id dead-ends" — **false**. What dead-ends it is the missing **action**, not the missing id. Verified both ways: `#/ViewItemCavidadMolde` → "no CRUD action"; `#/ViewItemCavidadMolde?action=Create` → renders the form.)* | browser + `ViewHome.kt:419,443,450,490`, `ConfigViewItem.kt:117-121,234-244` |
| F14 | **`urlCreate` omits the filter; the query path does not.** `navigateToQueryCreate(…, apiFilter)` builds an `ApiItem.Query.Create` and `apiItemToParamList` serializes `apiFilter` into the URL — so two *contextualized* creates have **different** URLs. `urlCreate` emits only `action=Create`. Using `urlCreate` for a destination's declared URL while its launch goes through `navigateToQueryCreate(apiFilter=…)` would make the two disagree — a declared URL that lies about where it goes, which is precisely what a documentation CI check would certify green. | `ConfigViewItem.kt:117-121,134-164,234-244` |
| F7 | **196** `ConfigView`s register (**15 plain / 79 item / 102 list**); ~92 are referenced from `ViewHome`. "Registered" and "referenced" are both **upper bounds** on "launchable". *(Corrected 2026-07-16: an earlier grep said "197 declared" — the extra one was `ConfigViewListRegistroHyeTallerIn`, inside a commented-out block at `ApiConfigViewImpl.kt:1423-1428`. Counting comments as code is how a findings register starts lying; the number now comes from the live registry, not from grep.)* | P1.1 measurement (runtime, `ViewRegistry`) |
