...
| **P4.4** | The search palette: fuzzy over label + keywords, launchable destinations only. Not AI — fuzzy handles typos and partial input; an LLM is a fallback for zero-lexical-overlap intent, if usage shows the need. **Catalog prerequisite done (mppArel `92f879a7`): `LaunchCatalog` grown from the 5-entry prototype to all 90 ViewHome destinations** (73 lists / 13 plain dashboards-boards-tableros / 4 fixed items; added the `LaunchSpec.Plain` variant), generated from ViewHome's nav targets classified against `ApiConfigViewImpl`, with a no-duplicate-key guard test. `keywords` seeded only where curated synonyms help; enriched incrementally. **Remaining for P4.4:** the Ctrl+K palette UI over `LaunchCatalog.specs`. | mppArel |
